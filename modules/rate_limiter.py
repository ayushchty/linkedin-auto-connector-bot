...
class RateLimiter:
    ...
    def _initialize_database(self):
        ...
        with self._get_connection() as conn:
            ...
            # Indexes for faster queries (must be separate statements in SQLite)
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_actions_week ON actions(week)')
    ...
    # ------------------------------------------------------------------
    # Compatibility helpers for older calling code
    # ------------------------------------------------------------------

    def can_send_request(self) -> bool:
        """
        Backwards-compatible helper used by the bot and CLI.

        Returns True if a new connection request can be sent under the
        current rate limits for CONNECTION_REQUEST, without blocking.
        """
        status = self.check_limits(ActionType.CONNECTION_REQUEST)
    ...
    def log_request(
        ...
        """
        Backwards-compatible helper to record a connection request.

        The `status` argument is kept for compatibility but currently
        only affects the `success` flag.
        """
        success = status.lower() in {"sent", "success", "successful", "ok"}
        self.record_action(
    ...
    def close(self) -> None:
        """
        Backwards-compatible no-op close method.

        The RateLimiter uses short-lived SQLite connections per operation,
        so there is no persistent connection to close, but main code calls
        this during cleanup.
        """
        # Nothing to clean up – method exists for API compatibility.
        logger.info("RateLimiter.close() called - no persistent resources to release")