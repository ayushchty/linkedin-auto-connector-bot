...
class BotConfig:
    """
    High-level configuration wrapper used by the bot and CLI.

    This class provides the interface (`from_file`, `to_dict`, `get`, `set`,
    `save`, etc.) expected by `main.py` and `cli.py` while delegating actual
    storage and validation to the underlying `Settings` object.
    """

    settings: Settings = field(default_factory=Settings)
    config_path: Optional[Path] = None

    # --- Properties consumed by the bot code ---
    @property
    def browser_type(self) -> BrowserType:
        return self.settings.browser.browser_type
    ...
    def log_dir(self) -> Path:
        return self.settings.logging.file_path.parent

    @property
    def session_dir(self) -> Path:
        # Default session directory; can be made configurable later
        return Path("sessions")

    @property
    def database_path(self) -> Path:
        # Use data database path from settings
        return self.settings.data.database_path

    @property
    def use_sessions(self) -> bool:
        # Simple flag for now; could be wired into Settings later
        return True

    # --- Factory methods ---
    @classmethod
    def from_file(cls, path: Union[str, Path]) -> "BotConfig":
        """
        Load configuration from a file (.env, .yaml/.yml, or .json).
        """
        path = Path(path)
        if not path.exists():
        ...
        if path.suffix == ".env":
            settings = Settings.load_from_env(path)
        elif path.suffix in {".yaml", ".yml"}:
            settings = Settings.load_from_yaml(path)
        elif path.suffix == ".json":
            settings = Settings.load_from_json(path)
        else:
            raise ValueError(f"Unsupported config file format: {path.suffix}")

        return cls(settings=settings, config_path=path)

    # --- Serialization helpers ---
    def to_dict(self) -> Dict[str, Any]:
        """Return a serializable dictionary representation of the configuration."""
        return self.settings.to_dict()

    # --- Simple key-based access used by the CLI ---
    def get(self, key: str, default: Any = None) -> Any:
        """
        Get a configuration value using dotted-path keys, e.g. 'browser.browser_type'.
        """
        data: Any = self.to_dict()
    ...
    def set(self, key: str, value: Any) -> None:
        """
        Set a configuration value using dotted-path keys.

        This updates the underlying `Settings` instance so changes can later
        be persisted via `save()`.
        """

        def _convert_value(current_value: Any, new_value: Any) -> Any:
            # Best-effort type conversion based on existing value type
            if isinstance(current_value, bool):
                return str(new_value).lower() in {"1", "true", "yes", "on"}
        ...
        def _set_on_obj(obj: Any, parts: List[str], new_value: Any) -> None:
            if not parts:
                return
            ...
            if len(parts) == 1:
                if hasattr(obj, attr):
                    ...
                    setattr(obj, attr, _convert_value(current, new_value))
                return

            if hasattr(obj, attr):
                _set_on_obj(getattr(obj, attr), parts[1:], new_value)

        _set_on_obj(self.settings, key.split("."), value)

    def save(self, path: Optional[Union[str, Path]] = None) -> None:
        """
        Persist configuration to disk.

        If no path is provided, it will:
        - Use the original `config_path` if available, otherwise
        - Default to 'config.json' in the project root.
        """
        target = Path(path) if path is not None else (self.config_path or Path("config.json"))
        target.parent.mkdir(parents=True, exist_ok=True)

        if target.suffix in {".yaml", ".yml"}:
            self.settings.save_to_yaml(target)
        else:
            # Default to JSON if extension is unknown or '.json'
            self.settings.save_to_json(target)

        self.config_path = target

    def get_message_template(self) -> str:
        """
        Get the connection message template content.

        If `linkedin.message_template_path` is configured and exists, the
        template is loaded from that file; otherwise a simple default is used.
        """
        template_path = self.settings.linkedin.message_template_path
        if template_path:
        ...
        return (
            "Hi {first_name}, I came across your profile and would love to "
